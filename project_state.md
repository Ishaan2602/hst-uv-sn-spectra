...
- [~] COS full custom extraction (NUV done, FUV next): installed `calcos` in the WSL env, synced COS refs (lref), ran the full custom extraction on SN2023ixf NUV G230L (lf8803d5q). `reduce_cos.py` = default, `reduce_cos_custom.py` copies the XTRACTAB, narrows the source box HEIGHT (57 -> 15), points a copy of the corrtag at it, re-runs calcos. Visualized in `cos_sandbox`. Finding: the narrow box roughly halves the flux scatter (NUVC std 4.84e-15 -> 2.49e-15) while preserving the continuum + Mg II 2800 emission. FUV (G140L) is the same workflow but needs XTRCTALG=BOXCAR (FUV defaults to TWOZONE).
...
- [~] **COS full custom extraction**: NUV G230L done (see section 2). calcos in WSL + XTRACTAB box editing working, noise roughly halved by the narrow box. FUV G140L still to do (set XTRCTALG=BOXCAR).
