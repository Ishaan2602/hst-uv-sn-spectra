"""
first-pass HST UV supernova reduction pipeline.

ties together the session's pieces into one driver:
  discover (classification query) -> pick a stis ccd epoch -> download -> reduce (wsl) -> coadd.

the heavy reduction (x1d / defringe) runs in the wsl surf_uv env via reduce_epoch.py, since
those wrap the hstcal binaries that don't exist on native windows. everything else is here.

run as a script for a demo on SN2023ixf, or import and call run('SNname').
"""
...
def pick_stis_ccd_epoch(target):
...
def coadd(ids, target, save=True):
    ...
    for g in ('g230lb', 'g430l', 'g750l'):
        ...
        stack.append(o); ax.plot(newax, o, lw=0.5, alpha=0.5, label=g.upper())
